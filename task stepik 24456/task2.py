...
objects = [1, 2, 1, 2, 3, True]  # будем считать, что одинаковые числа соответствуют одинаковым объектам,
unic_list = objects[0:1]
for obj in objects:  # доступная переменная objects
    len_unic_list = len(unic_list)
    for i in range(len(unic_list)):
        if obj is unic_list[i]:
            break
        else:
            if i == len_unic_list-1:
                unic_list.append(obj)
print(len(unic_list))

# пример решения в одну строку
